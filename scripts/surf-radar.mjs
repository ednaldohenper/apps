...
async function main(){
  ...
  if(dir && fs.existsSync(dir)){
    const p=path.join(dir,"Marketing","Instagram","Dados & Inteligência"); fs.mkdirSync(p,{recursive:true});
    fs.writeFileSync(path.join(p,"Radar de Surf (auto).md"),md);
    console.log(`Radar escrito no vault: Marketing/Instagram/Dados & Inteligência/Radar de Surf (auto).md (${md.length} chars · ${(ai&&ai.temas||[]).length} temas).`);
  } else {
